...
@Autonomous(name="AutoSpecimenParkRightTest", group="Autonomous")
        ...
        while (opModeIsActive()) {
            ...
            if (stepTwo) {
                if (myRobot.slide.SlideMotor1.getCurrentPosition() > myRobot.slide.slideStartingPosition + 100) {
                    ...
                    if (myRobot.slide.SlideMotor1.getCurrentPosition() >  700) {
                        stepFour = true;
                    }
                }
            }
            ...
            if (stepFour) {
                ...
                if (myRobot.slide.SlideMotor1.getCurrentPosition() <  150) {
                    stepFive = false;
                    stepEight =true;
                }
            }
            ...
            if (stepSeven) {
                ...
                //sleep(1000);
                if (!driveStarted) {
                    driveToRung = myRobot.driveTrain.driveStraightPT(params.defaultDrivePower * params.powerReduction, 2);
                    driveStarted = true;
                }
                ...
                if (driveToRung) {
                    ...
                    myRobot.driveTrain.stop();
                    //driveStraightReached = false;
                    telemetry.addData("StepSeven after", "drive reached " + driveToRung);
                    telemetry.update();
                    //myRobot.arm.endAutoArmPosition = myRobot.arm.getCurrentPosition(myRobot.arm.ArmMotor1);
                }
                //myRobot.arm.endAutoArmPosition = myRobot.arm.getCurrentPosition(myRobot.arm.ArmMotor1);
